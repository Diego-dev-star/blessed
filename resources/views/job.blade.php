                ...
                <div class="col-lg-12 col-sm-12">
                    <div class="all_folio_items row shafull_container">
                        @foreach($model as $JobPost)
                            <div class="blog_item shaf_item" data-groups='["all", "{{$JobPost->getTranslatedAttribute('category')}}"]'>
                                    ...
                                    <div class="bi_details">
                                        <div class="bi_meta">
                                            ...
                                            <a href="#" class="bi_cat"><i
                                                        class="pe-7s-tools"></i>{{$JobPost->getTranslatedAttribute('category')}}</a> |
                                            <a href="#" class="bi_cat"><i
                                                        class="pe-7s-wallet"></i>{{$JobPost->getTranslatedAttribute('salary'.'/Zl в час')}}
                                            </a> |
                                            <a href="#" class="bi_cat"><i
                                                        class="pe-7s-map-marker"></i> {{$JobPost->city}}
                                            </a>
                                        </div>
                                        <h3><a href="{{route('job.show',['id' => $JobPost->id, 'slug' => $JobPost->slug])}}">{{$JobPost->getTranslatedAttribute('title')}}</a></h3>
                                        <div class="bi_content">
                                            {{$JobPost->getTranslatedAttribute('ShortDescription')}}
                                        </div>
                                        <a href="{{route('job.show',['id' => $JobPost->id, 'slug' => $JobPost->slug])}}" class="ast_learnmore_2">@lang('lang.readmore')<i
