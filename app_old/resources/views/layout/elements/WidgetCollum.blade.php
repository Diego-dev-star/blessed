...
<div class="col-lg-4 col-sm-6 col-md-4 sidebar">
    ...
    <aside class="widget categories_list">
        <h3 class="widget_title">{{__('Категории')}}</h3>
        <ul>
           @foreach($categories as $key => $category)
               @if($key <= 3)
               <li><a href="{{Url( app()->getLocale()). '/categories/' .$category->id}}">{{$category->name}}</a></li>

                @endif
           @endforeach

               <div class="tabclouds clearfix">
                   <a href="{{Url( app()->getLocale() . '/categories')}}">{{__('Все категории')}}</a>
               </div>

        </ul>
    </aside>
    <aside class="widget">
        <h3 class="widget_title">{{__('Социальные сети')}}</h3>
    ...
    <aside class="widget">
        <h3 class="widget_title">{{__('Рекомендуемое')}}</h3>
        <div class="rc_posts">
